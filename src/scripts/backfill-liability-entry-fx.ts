...
async function planUpdates(platformCurrency: string): Promise<UpdatePlan[]> {
...
export async function runBackfillLiabilityEntryFx(options: BackfillOptions): Promise<BackfillSummary> {
  const startedAt = Date.now();
  const platformCurrency = await requirePlatformCurrency();
  const plans = await planUpdates(platformCurrency);

  const bySource: BackfillSummary["bySource"] = {
